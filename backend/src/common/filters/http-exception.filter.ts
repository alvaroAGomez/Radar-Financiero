...
export class HttpExceptionFilter implements ExceptionFilter {
  private readonly logger = new Logger(HttpExceptionFilter.name);
    ...
    // Log the error (Only error/warn details as per Backend Architecture guidelines)
    if (status >= 500) {
      this.logger.error(
      ...
      this.logger.warn(
        `HTTP ${status} - [${request.method}] ${request.url} - Warn: ${JSON.stringify(
          message
        )}`
      );
    }
